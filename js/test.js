...
function timeTests() {
	...
	for (var j=0;j<10;j++) {
	    results[i].full.push(timeChange(tests[i],j*3));
	    results[i].single.push(timeSingle(tests[i],j*3+1));
	}
	results[i].fullAvg = 0;
...
$(document).ready(function() {
    ...
    $("#output").append("ham}]");
    for (var i=0;i<2;i++) {
	$("#output").append("\\addplot coordinates {");
	for (var n in r) {
	    $("#output").append("("+n+","+ ( i==0 ? r[n].fullAvg : r[n].singleAvg ) +") ");
	}
	$("#output").append("};\n");
    }
