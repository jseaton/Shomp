...
function generateUI(node,structs,callback,name) {
	...
	if (match=node.type.match(/([bi]?)vec([234])/)) {
	    callback[name] = match[2]==2 ? new GLOW.Vector2() : (match[2]==3 ? new GLOW.Vector3() : new GLOW.Vector4());
		...
		if (match[2] > 2) {
			...
			onChange: function(hsb, hex, rgb, el) {
			    ...
			    callback[name].set.apply(callback[name],ns);
			    render();
			},
			...
			rs.css('backgroundColor', '#' + hex);
			callback[name].value[$(this).attr('n')] = parseFloat(this.value);
			render();
		    });
		}
		r.append(nextCell);
